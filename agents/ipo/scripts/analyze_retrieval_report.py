...
def _render_field_topk_block(
    ...
    *,
    ...
    for i, h in enumerate(hits, 1):
        ...
        w(
            f"| {i} | {h.get('page', '—')} | `{h.get('category', '—')}` | "
            f"{_md_escape(str(role_cn))} | {score_s} | `{_md_escape(src)}` | "
            f"`{stop}` | `{_md_escape(str(exp))}` |"
        )
    w("")
    w("**各条原文摘录：**" + ("（整表完整展示；已转为可渲染表格）" if unit == "table" else "（完整展示）"))
    w("")
    for i, h in enumerate(hits, 1):
        raw = h.get("excerpt") or ""
        body, mode = format_evidence_body(raw, limit=excerpt_limit)
...
def grade_field(field_code: str, hits: list[dict[str, Any]]) -> tuple[str, list[str]]:
    ...
    if is_table_unit:
        ...
        if top_cat != "table":
            score -= 3
            reasons.append("Top-1 不是 table，整表召回不完整")
        if score >= 8:
            return "A", reasons
        if score >= 5:
            return "B", reasons
...
def build_report(
    ...
    *,
    ...
    if not fin_rows and not leg_rows:
        ...
        w("")
        w(
            "策略：指标（REV/CFO 等）不再单独 Top-K；每种表保留多张候选整表"
            "（概要 / 附录一 / 讨论节），抽数交给下游。"
        )
    else:
        w(
