...
from src.graph.parallel import run_finance_legal_parallel, run_master_from_saved  # noqa: E402
...
async def _amain() -> int:
    ...
    try:
        if args.skip_experts:
            ...
            mkt = await MarketAgent(
                run_logger=market_logger, debate_dir=debate_dir, demo=True
            ).run(
                args.doc_id,
                issuer_type=args.issuer_type,
                parse_json=parse_json,
        ...
        if market_logger is not None and not market_logger._closed:
            market_logger.close(final_summary="market_demo")
    ...
    if "legal" in result:
        ...
        for b in (leg.get("score_breakdown") or [])[:10]:
            print(f"  +{b.get('delta')} {b.get('code')} ({b.get('rule_ref')}) {b.get('note') or ''}")
    if "market" in result:
        mkt = result["market"] or {}
        print(f"[market] score={mkt.get('risk_score')} demo={(mkt.get('features') or {}).get('demo')}")
        print(f"  summary: {mkt.get('summary')}")
