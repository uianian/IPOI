...
证据规则：
1. 只使用【己方已审计结果上下文】、己方 claim 已有证据和本轮补证 hits；市场结构化证据可以没有招股书页码，但必须保留可核验字段名、证据 ID、观察截止日或数值。只能使用已采集且不晚于 as_of_date 的本地市场证据；不得触发远程抓取，不得读取上市后数据。
...
5. 新证据只能用于验证、部分修订或否定被问到的市场主张；辩论文本不得擅自改写已审计 risk_score / 确定性风险分。确需重评分时，在 remaining_uncertainty 中明确要求重新运行市场评分。
