...
from src.retrieval.section_map import SECTION_MAP_VERSION
    ...
    async def build_from_parse(
        ...
        *,
        ...
        if not force and index_exists(self._index_root, doc_id):
            ...
            section_version = str((meta.extra or {}).get("section_map_version") or "")
            if (
                str(Path(meta.parse_json_path).resolve()) == abs_path
                and section_version == SECTION_MAP_VERSION
            ):
                ...
                return BuildResult(
                    ...
                    extra={
                        "section_map_version": section_version,
                        "section_count": len(
                            ((meta.extra or {}).get("section_map") or {}).get(
                                "section_spans"
                            )
                            or []
                        ),
            ...
            if section_version != SECTION_MAP_VERSION:
                logger.info(
                    "Rebuilding legacy index doc_id=%s: section_map_version=%r expected=%s",
                    doc_id,
                    section_version,
                    SECTION_MAP_VERSION,
                )
        ...
        meta = IndexMeta(
            ...
            extra={
                ...
                "section_map": stats.get("section_map") or {},
            },
        )
